...
$(document).ready(function(){
  ...
  ref.onSnapshot(querySnapshot => {
    querySnapshot.forEach(doc => {
      ...
      // ChartData.push({id:doc.id,ChartData:data2[0].Charts});
      newData.push({id:doc.id,data:doc.data()});
      // KH.push({Date:data2,id:doc.id});
    });
...
$(list).delegate(`tr`, `click`, function (e){
  ...
  database.on('value', function(snapshot) {
      ...
      for(var i in snapshot.val()){
        ...
        // time.push(snapshot.val()[i]);
        // snapshot.val()[i].sort(function (a,b){
        //   return a.time > b.time ? 1 : -1;
        // })
        document.querySelector('.show2').style.display="none";
        $show.append('<li>'+'<div class="">'+'<i class="fas fa-user-circle i-size" title="匿名"></i>：'+'<span class="list-p">'+snapshot.val()[i].content+'<span>'+'<br>'+'<small>'+snapshot.val()[i].time+'<small>'+'</div>'+'</li>');
      }
      // for(var k=0; k<time.length;k++){
      //   // console.log(time[k].content,time[k].time);
      //   
      //   str +='<li>'+'<div class="">'+'<i class="fas fa-user-circle i-size" title="匿名"></i>：'+'<span class="list-p">'+time[k].content+'<span>'+'<br>'+'<small>'+time[k].time+'<small>'+'</div>'+'</li>'
      //   // $show.prepend(str);
