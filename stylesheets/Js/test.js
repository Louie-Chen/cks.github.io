...
$(document).ready(function(){
  ...
  ref.onSnapshot(querySnapshot => {
    ...
    for(var i=0; i<data.length;i++){
      console.log(data[2].Charts);
      ChartData.push(data[2].Charts);
      ...
      "<td class='text-center' data-item="+data[i].name+">"+ChartData[i].BankDebt+'</td>'+
      "<td class='text-center' data-item="+data[i].name+">"+'XXX'+'</td>'+
      '</tr>'
...
function UpData(){
  var db = firebase.firestore();
  var ref = db.collection('個人資料').doc('律師1');
  // var Chart ={
  //   Real:"房地糾紛",
  //   litigation:"訴訟程序",
  //   Car:"車禍糾紛",
  ...
  // }
  var Chart =[
    {label:"房地糾紛",data:10,color:"#F3F3FA"},
    {label:"訴訟程序",data:15,color:"#E6E6F2"},
    {label:"車禍糾紛",data:15,color:"#D8D8EB"},
    {label:"詐騙案件",data:30,color:"#C7C7E2"},
    {label:"毒品問題",data:0,color:"#B8B8DC"},
    {label:"營造工程",data:80,color:"#A6A6D2"},
    {label:"公司經營",data:15,color:"#9999CC"},
    {label:"遺產繼承",data:15,color:"#8080C0"},
    {label:"政府採購",data:20,color:"#7373B9"},
    {label:"環境保護",data:10,color:"#5A5AAD"},
    {label:"保險爭議",data:5,color:"#5151A2"},
    {label:"選舉訴訟",data:5,color:"#484891"},
    {label:"金融市場",data:30,color:"#9F35FF"},
    {label:"勞資糾紛",data:60,color:"#921AFF"},
    {label:"消費爭議",data:80,color:"#8600FF"},
    {label:"感情事件",data:12,color:"#6F00D2"},
    {label:"銀行債務",data:15,color:"#5B00AE"},
    {label:"智慧財產",data:28,color:"#4B0091"},
    {label:"公平交易",data:50,color:"#3A006F"},
    {label:"性侵案件",data:78,color:"#28004D"},
    {label:"醫療糾紛",data:56,color:"#B15BFF"},
    {label:"兒少事件",data:88,color:"#BE77FF"},
    {label:"網路世界",data:30,color:"#CA8EFF"},
    {label:"國家賠償",data:20,color:"#DCB5FF"},
    {label:"支付命令",data:10,color:"#E6CAFF"}]
  ref.set({
    Charts:Chart
  },{merge: true}).then(() => {
    alert('資料成功更新');
